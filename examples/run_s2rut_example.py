...
fn = os.path.join("/mnt/t/data", "product_archive", "S2B", "S2_MSI_L1C", "2021", "07", "19", "eoio_S2B_MSIL1C_20210719T104619_N0500_R051_T31TCG_20230213T211917.nc")
# s2_l1c_ds = xr.open_dataset(f'{os.path.basename(s2a_product_path).split(".")[0]}.nc')
s2_l1c_ds = xr.open_dataset(fn)

import pickle
with open("/mnt/t/data/product_archive/S2B/S2_MSI_L1C/2021/07/19/eoio_S2B_MSIL1C_20210719T104619_N0500_R051_T31TCG_20230213T211917_ds_attrs.pkl", 'rb') as f:
    s2_l1c_ds.attrs.update(pickle.load(f))

for var in s2_l1c_ds:
    if os.path.exists(f"/mnt/t/data/product_archive/S2B/S2_MSI_L1C/2021/07/19/eoio_S2B_MSIL1C_20210719T104619_N0500_R051_T31TCG_20230213T211917_{var}_attrs.pkl"):
        with open(
                f"/mnt/t/data/product_archive/S2B/S2_MSI_L1C/2021/07/19/eoio_S2B_MSIL1C_20210719T104619_N0500_R051_T31TCG_20230213T211917_{var}_attrs.pkl",
                'rb') as f:
            s2_l1c_ds[var].attrs.update(pickle.load(f))
...
ds.attrs.update({
    'product_date': s2_l1c_ds.attrs['product_date'].strftime('%Y-%m-%d')
})
ds.attrs.pop('product_metadata')
for var in ds:
    if 'add_offset' in ds[var].attrs.keys():
        # Remove 'add_offset' from the variable's attributes
        ds[var].attrs.pop('add_offset')
    if 'scale_factor' in ds[var].attrs.keys():
        # Remove 'add_offset' from the variable's attributes
        ds[var].attrs.pop('scale_factor')
    if 'product_metadata' in ds[var].attrs.keys():
        ds[var].attrs.pop('product_metadata')

print(ds)

ds.to_netcdf(fn.replace('.nc', '_withUnc.nc'))
