...
for key, path in VAR_MTD_MAP.items():
    metadata[key] = {band: ds[band].attrs['product_metadata'][path] for band in ds.data_vars if 'B' in band}

print("Metadata extracted for uncertainty calculation:", metadata)
RUTl1 = S2RUT_L1(input_contributors)
u_ref, u_cont = RUTl1.unc_calculation_abs(ds['B01'].values, 'B01', 0, metadata, [ds['solar_zenith_angle'].values], do_contributor=True)
