...
class S2Processor:
    ...
    def get_data(self, get_va=False, doplot=False):
        ...
        if (
            mtd_eup["n1:Level-1C_User_Product"]["n1:Auxiliary_Data_Info"]["GRI_List"]
            is None
        ):
        ...
        for theband in self.selected_bands:
            ...
            if get_va == True:
                ...
                for i in range(len(view_angle_list)):
                    ...
                    if int(band_id) == S2bandID[theband]:
                        ...
                        VAA = mtd_tl["n1:Level-1C_Tile_ID"]["n1:Geometric_Info"][
                            "Tile_Angles"
                        ]["Viewing_Incidence_Angles_Grids"][i]["Azimuth"][
                            "Values_List"
                        ][
                            "VALUES"
                        ]
                        ...
                        VAA_LR[idet, :, :] = self.extrapole_2D_nearest(
                            VAA_LR[idet, :, :]
                        )
                ...
                VZA_HR = np.zeros((N, N))
                VZA_int = interpolate.RectBivariateSpline(
                    one_axis_LR, one_axis_LR, VZA_LR
                )
