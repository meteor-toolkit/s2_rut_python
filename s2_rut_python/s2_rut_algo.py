...
class S2RutAlgo:
    ...
    def unc_calculation(self, band_data, band_id, spacecraft):
        ...
        if self.unc_select[1]:
            self.u_stray_sys = 0.3 * rad_conf.Lref[band_id] / 100
        else:
            self.u_stray_sys = 0
        ...
        if self.unc_select[5]:
            self.u_DS = rad_conf.u_DS_all[spacecraft][band_id]
        else:
            self.u_DS = 0
        ...
        if self.unc_select[7]:
            self.u_diff_abs = rad_conf.u_diff_absarray[spacecraft][band_id]
        else:
            self.u_diff_abs = 0
