...
S2_BAND_SAMPLING = {'B01':60, 'B02':10, 'B03':10, 'B04':10, 'B05':20, 'B06':20, 'B07':20, 'B08':10, 'B8A':20, 'B09':60,
                    'B10':60, 'B11':20, 'B12':20}
S2_BAND_PIXELS = {'B01':1830, 'B02':10980, 'B03':10980, 'B04':10980, 'B05':5490, 'B06':5490, 'B07':5490, 
                  'B08':10980, 'B8A':5490, 'B09':1830, 'B10':1830, 'B11':5490, 'B12':5490}
...
class S2Processor:
    ...
    def write_unc(self, im, fpath, band, ds_profile):
        ...
        # quantize image
        im2 = ((im*self.metadatadict['quant']) + self.metadatadict['offset'][band]).astype('u2')
        im2[im==np.nan] = S2nodata
    ...
    def get_data(self, theband, get_va=False, doplot=False):       
        ...
        ## compute radiance L
        L = rho[xstart:xstop,ystart:ystop] * self.metadatadict['Esun'][theband] * self.metadatadict['Usun'] * np.cos(SZA_HR[xstart:xstop,ystart:ystop]*np.pi/180.) / np.pi    
    ...
    def get_unc(self, get_va=False, doplot=False):
        ...
        for ib, theband in enumerate(self.selected_bands):
            ...
            u_ref, u_sig, u_sys, u_cont = RUTl1.unc_calculation(L, theband, ib, self.metadatadict, self.sun_zenith)
