...
class TestS2RUT(unittest.TestCase):
    ...
    def test_run_rut_total_uncertainty(self, mock_unc_calculation, mock_get_band_unc_parameters):
    ...
    @mock.patch('s2_rut_python.s2_rut_interface.S2RUT.get_band_unc_parameters')
    @mock.patch('s2_rut_python.s2_rut_interface.MyS2RUTAlgo.unc_calculation')
    def test_run_rut_components_uncertainty(self, mock_unc_calculation, mock_get_band_unc_parameters):
        # Mock the return value of the unc_calculation method
        mock_unc_calculation.return_value = np.random.rand(5, 5)
        mock_get_band_unc_parameters.return_value = {
            'a': 1.0,
    ...
    @mock.patch("s2_rut_python.s2_rut_interface.S2RUT.get_band_unc_parameters")
    @mock.patch("s2_rut_python.s2_rut_interface.S2RUT.return_unc_correlations")
    def test_run_rut_invalid_unc_info(self, mock_return_unc_correlations, mock_get_band_unc_parameters):
        band_names = ["B01"]
        unc_info = "invalid"
        with self.assertRaises(ValueError):
            self.s2_rut.run_rut(self.mock_dataset, band_names, unc_info)
        mock_return_unc_correlations.assert_called_once()
