...
class S2RutOp:
    ...
    def initialise(self):
        ...
        self.spacecraft = self.datastrip_meta.getElement('General_Info').getElement('Datatake_Info').getAttributeString('SPACECRAFT_NAME')
        ...
        rut_product = snappy.Product(self.source_product.getName() + '_rut', 'S2_RUT',
                                     ...
                                     masterband.getRasterHeight())  # in-memory product
        ...
        self.rut_product_meta.addElement(sourceelem)
        # RUT_VERSION
        version    = snappy.GPF.getDefaultInstance().getOperatorSpiRegistry().getOperatorSpi("S2RutOp").getOperatorDescriptor().getVersion()
    ...
    def computeBand(self,bandu):
        ...
        for resolution in np.arange(3):
            ...
            if pprod["product"]==source_band.getProduct():
                cloudmask  = self.mask_roi('opaque_clouds_'+pprod["product_name"].split("_")[0],
                                           0,0,bw,bh)
                cirrusmask = self.mask_roi('cirrus_clouds_'+pprod["product_name"].split("_")[0],
                                           0,0,bw,bh)
                break
        ...
        try:    assert szah==bh
        except: raise IOError("Heigth of band and solar zenith arrays are not the same")
        try:    assert szaw==bw
        except: raise IOError("Width of band and solar zenith arrays are not the same")

        sza_samples = source_sza.readPixels(0,0,szaw,szah,
                                            np.zeros(szaw*szah))
        ...
        unc = self.rut_algo.unc_calculation(np.array(toa_samples,dtype=np.float64),
                                            toa_band_id,self.spacecraft)
